...
#
# Mail::TestMailer は response を返すことがないため、それを返すようにするためのクラス。
# 返却する response は成功（ステータス 250 ）を表します。
class TestMailerRespondOk < Mail::TestMailer
  # override
  def deliver!(mail)
...
#
# Mail::TestMailer は response を返すことがないため、それを返すようにするためのクラス。
# 返却する response は失敗（ステータス 400 ）を表します。
class TestMailerRespondNg < Mail::TestMailer
  # override
  def deliver!(mail)
    Mail::SmtpEnvelope.new(mail)
    Mail::TestMailer.deliveries << mail
...
# Mail::TestMailer は response を返すことがないため、それを返すようにするためのクラス。
# 返却する response は成功（ステータス 250 ）を表します。
# ただし含まれるメッセージ長がカラムサイズを超えているケース。
class TestMailerRespondOkButTooLong < Mail::TestMailer
  # override
  def deliver!(mail)
    Mail::SmtpEnvelope.new(mail)
    Mail::TestMailer.deliveries << mail
