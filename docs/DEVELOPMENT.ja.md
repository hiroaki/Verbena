...
本プロジェクトでは Kamal を利用したデプロイ方法を提供します。

### 構成

Kamalを用いて以下の3つのロール（サービス）をデプロイします。各ロールは同じイメージを利用し、環境変数で役割を分離しています。

| ロール   | 説明 |
|----------|-----------------------------------------|
| web      | 内部API専用のWebサーバ。`VERBENA_ADMIN_ROUTES_ONLY=0` で起動しますが、外部公開せず、他コンテナからのみアクセス可能です。 |
| admin    | 管理画面専用Webサーバ。`VERBENA_ADMIN_ROUTES_ONLY=1` で起動し、管理画面ルートのみ有効。23000番ポートをホストに公開し、外部から管理画面にアクセスできます（Basic認証等で保護）。 |
| worker   | バックグラウンドジョブ実行用のワーカー。APIや管理画面とは独立して動作します。 |
...
事故防止のため `require_destination: true` を有効化しているため kamal コマンドの実行時にはオプション `-d staging` の指定が必要です。
