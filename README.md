...
- **本番環境**: Docker secrets、環境変数管理システム、またはKubernetesのSecretを使用してデータベース認証情報を安全に管理してください。平文でのパスワード保存は避けてください。
...
このプロジェクトでは `docker-entrypoint-initdb.d` メカニズムを使用してデータベースの初期化を行います。

- **初期化スクリプト**: `./initdb/00-create-db-users.sh`
